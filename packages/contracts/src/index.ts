...
export const messageSchema = z.object({
  ...
  body: z.string(),
  humanOnly: z.boolean(),
  author: z.object({ id: z.string(), name: z.string(), type: z.enum(['human', 'bot', 'system']) }),
  createdAt: z.string(),
...
export function projectContext(
  history: Message[],
  options: { cursor: number; triggerSeqs: number[]; maxMessages: number; maxChars: number },
) {
  const eligible = history
    .filter((m) => !m.humanOnly && m.seq > options.cursor && m.author.type !== 'system')
  ...
  if (chars > options.maxChars || chosen.length > options.maxMessages)
    throw new Error('Current requests exceed the context limit. Split the request.');
...
export const outcomeSchema = z
  .object({ kind: z.enum(['reply', 'wait', 'silent']), text: z.string().max(16000) })
  .strict()
  .superRefine((v, c) => {
    if (v.kind !== 'silent' && !v.text.trim())
      c.addIssue({ code: 'custom', message: 'Reply or question cannot be empty.' });
    if (v.kind === 'silent' && v.text !== '')
      c.addIssue({ code: 'custom', message: 'Silent output must contain no text.' });
  });
...
export const hostEventSchema = z
  .object({
    ...
    activity: z.enum(['command', 'fileChange', 'tool', 'compacting']).optional(),
  })
  .strict();
export type HostEvent = z.infer<typeof hostEventSchema>;
export const hostInboundSchema = z.discriminatedUnion('type', [
  hostEventSchema,
  z
    .object({
      type: z.literal('heartbeat'),
      version: z.literal(PROTOCOL_VERSION),
      setupError: z.string().max(300).nullable(),
      projects: z.array(z.object({ localId: id, label, available: z.boolean() }).strict()).max(100),
    })
    .strict(),
]);
export const hostCommandSchema = z.discriminatedUnion('type', [
  dispatchSchema,
  z.object({ type: z.literal('cancel'), dispatchId: id, epoch: z.number().int() }),
  z.object({ type: z.literal('eventAck'), eventId: id }),
  z.object({ type: z.literal('reconcile'), dispatchIds: z.array(id) }),
