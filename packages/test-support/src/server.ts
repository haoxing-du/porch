...
export async function createTestApp() {
  ...
  const url = process.env.TEST_DATABASE_URL ?? 'postgres://localhost/postgres';
  const admin = pool(url);
  await admin.query(`CREATE SCHEMA ${schema}`);
  const db = pool(url, schema);
  await migrate(db);
  const uploadDir = await mkdtemp(join(tmpdir(), 'porch-test-'));
  const cfg: Config = {
    databaseUrl: url,
  ...
  const fixture = {
    ...
    async restart() {
      ...
      this.app = await createApp(db, cfg);
...
export async function login(app: FastifyInstance, name: 'alex' | 'sam' | 'outsider') {
  const response = await app.inject({ method: 'POST', url: '/api/auth/dev', payload: { name } });
