...
const message = (seq: number, body = `message ${seq}`, humanOnly = false) => ({
  id: `m${seq}`,
  seq,
  body,
  humanOnly,
  author: { id: 'alex', name: 'Alex', type: 'human' as const },
...
describe('human-only boundary', () => {
  ...
  it('uses one bounded eligible projection for initial, catch-up, resume and fresh paths', () => {
    const canary = {
      ...message(2, 'HUMAN_ONLY_CANARY', true),
      mentions: [{ id: 'SECRET_BOT', type: 'bot' as const }],
      attachments: [{ id: 'SECRET_FILE', name: 'SECRET_FILE', mediaType: 'text/plain', size: 2 }],
    };
    for (const cursor of [0, 1, 2]) {
      const input = projectContext([message(1), canary, message(3)], {
        cursor,
        triggerSeqs: [3],
        maxMessages: 50,
        maxChars: 24000,
  ...
  it('trims oldest background and reports omissions while keeping the request once', () => {
    const output = projectContext(
      ...
      { cursor: 0, triggerSeqs: [60], maxMessages: 50, maxChars: 100 },
    );
  ...
  it('rejects a request that cannot fit rather than dropping it', () => {
    expect(() =>
      projectContext([message(1, 'long request')], {
        cursor: 0,
        triggerSeqs: [1],
        maxMessages: 50,
        maxChars: 2,
