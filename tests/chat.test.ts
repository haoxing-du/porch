...
beforeAll(async () => {
  ...
  alex = await login(fixture.app, 'alex'); sam = await login(fixture.app, 'sam'); outsider = await login(fixture.app, 'outsider');
});
afterAll(async () => {await fixture?.close();});
describe('persistent authorized chat', () => {
  it('creates a workspace, joins with an invite, and creates a topic', async () => {
    const w = await call(fixture.app, alex, 'POST', '/api/workspaces', {name: 'Friends'}); workspace = w.id;
    ...
    const channel = await call(fixture.app, sam, 'POST', `/api/workspaces/${workspace}/channels`, {name: 'builds'});
    topic = (await call(fixture.app, sam, 'POST', `/api/channels/${channel.id}/topics`, {title: 'Recipe app'})).id;
  });
  it('orders concurrent messages and deduplicates retried sends', async () => {
    ...
    await Promise.all(Array.from({length: 8}, (_, i) => call(fixture.app, sam, 'POST', `/api/topics/${topic}/messages`, {body: `reply ${i}`, clientKey: crypto.randomUUID()})));
    const history = await call(fixture.app, sam, 'GET', `/api/topics/${topic}/messages`);
    expect(history.messages).toHaveLength(9);
    expect(history.messages.map((m: {seq: number}) => m.seq)).toEqual([1,2,3,4,5,6,7,8,9]);
    await fixture.restart();
    expect((await call(fixture.app, sam, 'GET', `/api/topics/${topic}/messages`)).messages).toHaveLength(9);
  });
  it('denies non-members history, subscriptions, sends, and uploads', async () => {
    for (const url of [`/api/topics/${topic}/messages`, `/api/workspaces/${workspace}/snapshot`, `/api/workspaces/${workspace}/events`]) {
      expect((await fixture.app.inject({method: 'GET', url, headers: {cookie: outsider}})).statusCode).toBe(403);
    }
    expect((await fixture.app.inject({method: 'POST',url: `/api/topics/${topic}/messages`,headers:{cookie: outsider},payload:{body:'intrude',clientKey:crypto.randomUUID()}})).statusCode).toBe(403);
  });
  it('persists human-only messages without treating near matches as prefixes', async () => {
    const sent = await call(fixture.app, alex, 'POST', `/api/topics/${topic}/messages`, {body: ' /nobots a secret',clientKey:crypto.randomUUID()});
    expect(sent.humanOnly).toBe(true); expect(sent.body).toBe('a secret');
    const ordinary = await call(fixture.app, alex, 'POST', `/api/topics/${topic}/messages`, {body:'/nbfoo',clientKey:crypto.randomUUID()});
  ...
  it('rejects revoked invites and removes access for a removed member', async () => {
    ...
    expect((await fixture.app.inject({method:'POST',url:'/api/invites/join',headers:{cookie:outsider},payload:{token:invite.token}})).statusCode).toBe(410);
    ...
    expect((await fixture.app.inject({url:`/api/topics/${topic}/messages`,headers:{cookie:sam}})).statusCode).toBe(403);
  });
});
