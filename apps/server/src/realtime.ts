  ...
  app.get(
    ...
    (socket, req) => {
      ...
      let cursor = z.coerce
        ...
        .nonnegative()
        .catch(0)
        .parse((req.query as { after?: string }).after);
      let busy = false;
      const timer = setInterval(async () => {
        ...
        try {
          await member(db, (await user(db, req)).id, workspace);
          if (socket.bufferedAmount > 1048576) {
            socket.close(1013, 'Reconnect to load new messages.');
            return;
          }
          const events = (
            await db.query(
              'SELECT id::text,kind,topic_id FROM events WHERE workspace_id=$1 AND id>$2 ORDER BY id LIMIT 100',
              [workspace, cursor],
            )
          ).rows;
          if (events.length) {
