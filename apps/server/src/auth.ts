...
export async function authRoutes(app: FastifyInstance, db: pg.Pool, cfg: Config) {
  async function signIn(
    ...
    const person = (
      await db.query(
        'INSERT INTO users(id,identity,name,avatar) VALUES($1,$2,$3,$4) ON CONFLICT(identity) DO UPDATE SET name=excluded.name,avatar=excluded.avatar RETURNING id',
        [uid(), identity, name, avatar],
      )
    ).rows[0];
    const token = secret();
    await db.query(
  ...
  app.get('/api/me', async (req) => {
    let person;
    try {
      person = await user(db, req);
    } catch (e) {
      if (!(e instanceof Problem) || e.statusCode !== 401) throw e;
    }
    return {
      user: person ?? null,
      devAuth: cfg.devAuth,
      ...
      workspaces: person
        ? (
            await db.query(
              'SELECT w.* FROM workspaces w JOIN memberships m ON m.workspace_id=w.id WHERE m.user_id=$1 ORDER BY w.name',
              [person.id],
            )
          ).rows
        : [],
  ...
  if (cfg.devAuth)
    app.post('/api/auth/dev', async (req, reply) => {
      ...
      await signIn(`dev:${name}`, name[0].toUpperCase() + name.slice(1), null, reply);
      return { ok: true };
    });
  app.post('/api/auth/logout', async (req, reply) => {
    if (req.cookies.porch)
      await db.query('DELETE FROM auth_sessions WHERE token_hash=$1', [hash(req.cookies.porch)]);
    reply.clearCookie('porch', { path: '/' });
    return { ok: true };
  });
  if (cfg.githubId && cfg.githubSecret) {
    await app.register(oauth, {
      name: 'githubOAuth2',
      credentials: {
        client: { id: cfg.githubId, secret: cfg.githubSecret },
        auth: oauth.GITHUB_CONFIGURATION,
      },
      startRedirectPath: '/api/auth/github',
      callbackUri: `${cfg.origin}/api/auth/github/callback`,
      scope: ['read:user'],
      cookie: { secure: cfg.production, httpOnly: true, sameSite: 'lax' },
    });
